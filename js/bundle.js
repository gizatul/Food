...
function calc() {
  ...
  var sex, height, weight, age, ratio; //объ-е переменных и установка некоторым знач-я по умолчанию

  //Условие для localStorage
  if (localStorage.getItem('sex')) {
    //если в localStorage присутствует значение sex
  ...
  if (localStorage.getItem('ratio')) {
    //если в localStorage присутствует значение ratio
  ...
  function getStaticInformation(selector, activeClass) {
    var elements = document.querySelectorAll(selector);
    elements.forEach(function (item) {
      //в данном случае лучше использовать forEach, а не делегирование событий, для избежания багов с щелком не на кнопку
...
function cards() {
  ...
  var MenuCard = /*#__PURE__*/function () {
    function MenuCard(src, alt, title, descr, price, parentSelector) {
      ...
      //classes- rest-оператор, т.к неизвестно сколько классов в будущем мб добавлено (вместо .menu__item) 
    ...
    _createClass(MenuCard, [{
      ...
      value: function render() {
        // ф-я добавления на страницу
        var elementMenu = document.createElement('div'); //создание элемента с тегом div
        if (this.classes.length == 0) {
          //создаем условие для значения по умолчанию, если длина массива 0(т.е массив пустой, то добавляем .menu__item)
          this.classes = 'menu__item';
          elementMenu.classList.add(this.classes);
        } else {
          this.classes.forEach(function (className) {
            return elementMenu.classList.add(className);
          });
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_14__["getResourсe"])('http://localhost:3000/menu').then(function (data) {
    //data - данные которые придут из localhost:3000
    data.forEach(function (_ref) {
      var img = _ref.img,
        ...
        descr = _ref.descr,
        price = _ref.price;
      //перебор и {}-деструктуризация объектов в массиве data по частям
...
/* harmony import */ var _services_services__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ../services/services */ "./js/services/services.js");
 ...
 //импорт ф-ий показа и закрытия окна из modal

function forms(formSelector, modalTimer) {
  //5. Формы отправки (POST)
  var forms = document.querySelectorAll(formSelector); //выдергиваем формы
  var message = {
    // список фраз после отправки формы
  ...
  forms.forEach(function (item) {
    // применяем ф-ю postData для каждой ф-ии
    bindPostData(item);
  });
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      ...
      var statusMessage = document.createElement('img'); //создаем элемент на странице показывающий рис загрузки
      ...
      var formData = new FormData(form); //создаем объект FormData, для формирования данных кот-е отправляет пользователь, формируется в формате ключ-значение. Внутри form, кот-й получим от пользователя. В html в форме обязательно должен быть атрибут name=""
      var json = JSON.stringify(Object.fromEntries(formData.entries())); //перебор fromData для преобразования в json

      (0,_services_services__WEBPACK_IMPORTED_MODULE_10__.postData)('http://localhost:3000/requests', json) //вызов ф-ии отправки данных
      .then(function (data) {
        console.log(data);
  ...
  function showThanksModal(message) {
    ...
    (0,_modal__WEBPACK_IMPORTED_MODULE_9__.showModalWindow)('.modal', modalTimer); //снова показываем окно с новым содержимым

    var thanksModal = document.createElement('div'); //создаем блок с новым контентом
    ...
    setTimeout(function () {
      // удаляем ч/з 4сек наш рукотоворный блок и делаем так чтобы появлялось старое окно
      ...
      (0,_modal__WEBPACK_IMPORTED_MODULE_9__.closeModalWindow)('.modal'); // закрываем окно для пользователя. '.modal'-селектор модального окна кот-й закрываем
    }, 4000);
  }
...
function showModalWindow(modalSelector, modalTimer) {
  //аргумент будет тянуться из script.js
  var modalWindow = document.querySelector(modalSelector);
  modalWindow.classList.add('show');
  modalWindow.classList.remove('hide');
  document.body.style.overflow = 'hidden'; //блокировка прокрутки
  if (modalTimer) {
    //если есть modalTimer, то запускаем очистку таймера
...
function modal(triggerSelector, modalSelector, modalTimer) {
  //3. Modal window
  var btnContact = document.querySelectorAll(triggerSelector),
  ...
  btnContact.forEach(function (item) {
    item.addEventListener('click', function () {
      return showModalWindow(modalSelector, modalTimer);
    }); //стрелочная ф-я для обертки showModalWindow. 
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === "Escape" && modalWindow.classList.contains('show')) {
      //если нажата кнопка Esc и модал окно открыто, то
      closeModalWindow(modalSelector);
    }
  });
  ...
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      // если прокрученная часть + видимая часть >= полной прокрутке (т.е полному сайту), то покажи модалку
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (modal);
 //экспорт ф-ии закрытия окна
 //экспорт ф-ии открытия окна
...
function slider(_ref) {
  ...
  function activeDot() {
    ...
    }); //назначение всем точкам прозрачности .5
    dotsArray[slideIndex - 1].style.opacity = 1; //
  }

  addZeroInCurrent();
  slides.forEach(function (slide) {
    //для ширину каждого слайда приравниваем к ширине wrapper
  ...
  var dotsArray = []; //для трансформации dots из псевдомассива в массив
  ...
  prevBtn.addEventListener('click', function () {
    ...
    slidesField.style.transform = "translateX(-".concat(offset, "px)"); //перелистывание

    if (slideIndex == 1) {
      // изменение цифры в меньшую сторону
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    // ставим 0 по умолчанию, чтобы вначале был 1-й таб
  ...
  tabsParent.addEventListener('click', function (e) {
    if (e.target && e.target.classList.contains(tabsSelector.slice(1))) {
      //делегирование события, клик по табу. slice применяется для удаления точки в начале, т.к. добавляем класс, а не селектор
      tabs.forEach(function (item, i) {
        //перебор, передача 2-х элементов (item - каждый таб кот-й перебираем, i - номер элемента по порядку который перебираем) 
        if (e.target == item) {
          // если e.target, тот элемент, на который мы кликнули совпадает с элементом, который перебираем в цикле forEach, то вызываем ф-ии ниже 
          hideTabContent();
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
      // вычитание миллисек от дедлайна тек. даты
      days = Math.floor(t / (1000 * 60 * 60 * 24)),
      // расчет кол-ва дней. Math.floor() - округление в меньш. сторону
      hours = Math.floor(t / (1000 * 60 * 60) % 24),
      //вычисляем остаток часов с помощью %
      minutes = Math.floor(t / 1000 / 60 % 60),
      // аналогично выше
      seconds = Math.floor(t / 1000 % 60); // аналогично выше
    if (t <= 0) {
      // для того чтобы было 00:00 после окончания акции
      days = 0;
      hours = 0;
      minutes = 0;
      seconds = 0;
    ...
    return {
      //возврат переменных наружу в виде объекта
      'total': t,
      // общее кол-во мс, нужно в будущем чтобы недопустить отриц. значение (под это будет ф-я updateClock())
      'days': days,
      // возврат наружу полученных значений
      'hours': hours,
      'minutes': minutes,
      'seconds': seconds
    };
  ...
  function setClock(selector, endtime) {
    //selector-глав. элемент, в будущ. это будет '.timer'
    var timer = document.querySelector(selector),
      // selector передали из аргумента ф-ии setClock 
      days = timer.querySelector('#days'),
      // вытягиваем id из страницы html
    ...
    function updateClock() {
      var t = getTimeRemaining(endtime); //назначаем на t, объект, полученный в рез-те ф-ии getTimeRemaining(), endtime-дедлайн, кот-й будем передавать
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "getResourсe": () => (/* binding */ getResourсe),
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _context.next = 2;
          return fetch(url, {
            //код ожидает выполнения fetch(благодаря await). Fetch возращает промис
            method: 'POST',
            // метод
            headers: {
              // заголовки
...
//Ф-я GET-запроса
var getResourсe = /*#__PURE__*/function () {
  ...
  return function getResourсe(_x3) {
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalTimer = setTimeout(function () {
    return (0,_modules_modal__WEBPACK_IMPORTED_MODULE_5__.showModalWindow)('.modal', modalTimer);
  }, 50000);
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_1__["default"])('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active'); //вызов ф-й
  (0,_modules_calc__WEBPACK_IMPORTED_MODULE_2__["default"])(); //вызов ф-й
  (0,_modules_cards__WEBPACK_IMPORTED_MODULE_3__["default"])(); //вызов ф-й
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_4__["default"])('form', modalTimer); //вызов ф-й
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_5__["default"])('[data-modal]', '.modal', modalTimer); //вызов ф-й
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2023-12-12'); //вызов ф-й
});
})();
